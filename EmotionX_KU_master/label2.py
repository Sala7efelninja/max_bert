...
def label(test_json_path):
    ...
    model.load_state_dict(torch.load(hparams.save_dir + '/' + hparams.model_name + '.pt'))
    # model.cuda()
    model.eval()
    pred_list = []
    ...
    for dialog in json_data:
        for utter_dict in dialog:
            ...
            pred_list.pop(0)
    json.dump(json_data, open(hparams.train_dir + '/result2.json', 'w'), indent=4, sort_keys=True)
